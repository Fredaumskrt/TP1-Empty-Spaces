...
public class Arquivo<T extends Registro> {
  ...
  private List<Long> espacosLivres = new ArrayList<>();
  ...
  public int create(T obj) throws Exception {
    ...
    arquivo.seek(encontraEspacosVazios(tam));
  ...
  public boolean update(T novoObj) throws Exception {
    ...
    if (endereco != -1) {
      ...
      if (tam2 <= tam) {
        ...
        arquivo.writeByte('*');
        espacosLivres.add(endereco);
        arquivo.seek(encontraEspacosVazios(tam2));
  ...
  private void recuperaEspacosVazios() throws IOException {
    espacosLivres.clear(); // Limpa a lista de espaços livres
    long tamanhoArquivo = arquivo.length();
    long posicaoAtual = 0;

    // Percorre o arquivo até o final
    while (posicaoAtual < tamanhoArquivo) {
      arquivo.seek(posicaoAtual);
      byte lapide = arquivo.readByte();
      if (lapide == '*') {
        // Se encontrarmos um byte marcado como livre, adicionamos seu offset à lista de espaços livres
        espacosLivres.add(posicaoAtual);
      }
      // Move para a próxima posição no arquivo
      posicaoAtual++;
    }
  }

  private long encontraEspacosVazios(int tamanhoNecessario) throws IOException {
    // Percorre a lista de espaços livres para encontrar um espaço adequado
    for (long espaco : espacosLivres) {
      int tamanhoRegistro = recuperaTamanhoDoRegistro(espaco);
      if (tamanhoRegistro == tamanhoNecessario) {
        // Se encontrarmos um espaço livre do tamanho do novo registro, retornamos o espaço livre
        espacosLivres.remove(espaco); // Remove o espaço livre da lista
        return espaco;
      }
    }
    // Se não encontrarmos um espaço livre adequado, retornamos o final do arquivo
    return arquivo.length();
  }

  private int recuperaTamanhoDoRegistro(long posicao) {
    try {
      ...
      arquivo.readByte(); // Ignora o byte de lápide
      short recordSize = arquivo.readShort();
      // Retorna o tamanho do registro
      arquivo.seek(currentPosition); // Retorna para a posição original do arquivo
      return recordSize;
    } catch (IOException e) {
      e.printStackTrace();
